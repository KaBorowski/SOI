 ...
 *===========================================================================*/
  ...
  while(rp->act_pri <= tmp->p_nextready->act_pri && tmp->p_nextready != NIL_PROC){
	tmp = tmp->p_nextready;
  }
  ...
  else if (tmp->p_nextready == NIL_PROC){
 ...
 */
  register struct proc *tmp, *tmp_head;
 ...
 *==========================================================================*/
  ...
  /*starzenie*/
  if (rdy_head[USER_Q]->base_pri >= MIN_PRI && rdy_head[USER_Q]->base_pri <= MAX_PRI){
	  while (tmp->p_nextready != NIL_PROC){
		tmp = tmp->p_nextready;
		if (tmp->act_pri < MAX_PRI && tmp->act_pri >= MIN_PRI)
			++(tmp->act_pri);
	  }
  }
  /*starzenie*/
  tmp = rdy_head[USER_Q];
  while(tmp->p_nextready != NIL_PROC && rdy_head[USER_Q]->act_pri <= tmp->p_nextready->act_pri){
	tmp = tmp->p_nextready;
  }

  if (tmp->p_nextready == NIL_PROC){
	rdy_tail[USER_Q]->p_nextready = rdy_head[USER_Q];
	rdy_tail[USER_Q] = rdy_head[USER_Q];
	rdy_head[USER_Q] = rdy_head[USER_Q]->p_nextready;
	rdy_tail[USER_Q]->p_nextready = NIL_PROC;	
  	...
  	tmp_head = rdy_head[USER_Q]->p_nextready;
	rdy_head[USER_Q]->p_nextready = tmp->p_nextready;
	tmp->p_nextready = rdy_head[USER_Q];
	rdy_head[USER_Q] = tmp_head;
  }
