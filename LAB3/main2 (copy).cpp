...
struct Semaphores * getSemaphores()
{
    ...
    if(shmid == 0)
            shmid = shmget(IPC_PRIVATE,sizeof(struct Semaphores) + M * sizeof(sem_t),SHM_W|SHM_R);
...
{/*funkcja przydziela buforom miejsce w pamieci wspoldzielonej / zwraca polozenie w pamieci wspoldzielonej */
    ...
    if(shmid == 0)
    {
        shmid = shmget(IPC_PRIVATE, M * sizeof(struct Buffer) + M * BUFFER_SPACE * sizeof(unsigned int), SHM_W|SHM_R);//IPC_PRIVATE zapewnia niepowtarzalny identyfikator
    }
