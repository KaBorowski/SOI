 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *biggest_hole, *prev_biggest_hole;
  ...
  if (!isWorstFit){
	...
	hp = hole_head;
	if (hp == NIL_HOLE) return (NO_MEM);
	biggest_hole = hp;
	prev_biggest_hole = NIL_HOLE;
	while (hp != NIL_HOLE && hp->h_base < swap_base){
		if(hp->h_len > biggest_hole->h_len){
			prev_biggest_hole = prev_ptr;
			biggest_hole = hp;
		}
		prev_ptr = hp;
		hp = hp->h_next;
	...
	if (hp->h_len >= clicks) {
		/* We found the biggest hole that is big enough.  Use it. */
		old_base = biggest_hole->h_base;	/* remember where it started */
		biggest_hole->h_base += clicks;	/* bite a piece off */
		biggest_hole->h_len -= clicks;	/* ditto */

		/* Delete the hole if used up completely. */
		if (biggest_hole->h_len == 0) del_slot(prev_biggest_hole, biggest_hole);
...
int do_hole_map(){
  ...
  unsigned int number = mm_in.m1_i1;
  unsigned int pairNumber;
  char * buff = mm_in.m1_p1;
  unsigned int pid = mm_in.m_source;
  int i = 0;
  ...
  if (nbytes%2 == 0) number--;

  pairNumber = number; /*liczba par (rozmiar, adres), ktore chcemy przeslac*/

  if (hp == NIL_HOLE) return 0;

  while (hp != NIL_HOLE && number > 0 && hp->h_base < swap_base){
  	buffer[i] = hp->h_len;
	buffer[++i] = hp->h_base;
	hp = hp->h_next;
	number--;
  ...
  sys_copy(0, D, (phys_bytes)buffer, pid, D, (phys_bytes)buff, nbytes);
 ...
 *===========================================================================*/
int do_worst_fit(){
  if (mm_in.m1_i1 == 0) isWorstFit = 0;
  else if (mm_in.m1_i1 == 1) isWorstFit = 1;
